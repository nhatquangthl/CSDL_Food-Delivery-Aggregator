...
- **Order State Processing:** Kiểm soát chặt chẽ trạng thái đơn hàng (PENDING -> CONFIRMED -> PREPARING -> READY_FOR_PICKUP -> PICKED_UP -> DELIVERED) tuân thủ tính chất ACID.
- **Dynamic Pricing Snapshot:** Lưu trữ ảnh chụp giá trị món ăn tại thời điểm đặt hàng để đảm bảo tính toàn vẹn tài chính.
- **Spatial & Status Querying:** Xử lý điều hướng tài xế dựa trên vị từ trạng thái (`is_active_order = TRUE/FALSE`).
